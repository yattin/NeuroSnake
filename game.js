...
const tileCount = 20; // 网格行列数 (canvas.width / gridSize)
...
function init() {
    ...
    updateScoreboard(); // Includes updating high score display now
    setupEventListeners();
    resetLevel();
    initializeSnake();
    createFood();
...
function startGame() {
    if (gameState === 'initial' || gameState === 'gameOver') {
        ...
        score = 0;
        updateScoreboard();
        resetLevel(); // Reset to level 1 on new game
        initializeSnake();
        createFood();
        ...
        // 清除旧的循环 (如果存在)
        // if (gameLoopId) { // gameLoopId is now for setTimeout, not requestAnimationFrame
...
function toggleGameMode() {
    if (currentMode === 'manual') {
        currentMode = 'ai';
        modeSwitchBtn.textContent = '切换至手动模式';
        console.log("Switched to AI mode");
        // 在后续任务中，这里会调用 AI 控制逻辑
    } else {
        currentMode = 'manual';
        modeSwitchBtn.textContent = '切换至 AI 模式';
